...
- [Docker, Vagrant, VMs, Cloud, OpenStack](#docker-vagrant-vms-cloud-openstack) 【important】
- [Notebook programming](#notebook-programming)
- [GitHub](#github)

## Keyboard remapping【important】
...
## Hammerspoon (desktop automation on macOS)【自动化一切】
...
```bash
docker images # checks those images
docker ps # checks the running containers, `-a` for all
docker stop <my_contariner>
docker start <my_con>
docker exec -it <my_con> /bin/bash
docker rm <my_con>

# for init a container the 1st time:
nvidia-docker run -it --name 'hz_varlidar' --shm-size 16g -v /data0/dataset/:/data -v /home/ljm/hz:/home/hz --dns=8.8.8.8 kom4cr0/cuda11.7-pytorch1.13-mamba1.1.1:1.1.1 /bin/bash 

# for pack up the running container to image, the transport the image to another machine
## at the local
docker commit hz_varlidar hz_varlidar_image:1.0
docker save -o hz_varlidar_image.tar hz_varlidar_image:1.0
gzip hz_varlidar_image.tar # optional
scp hz_varlidar_image.tar.gz user@目标服务器IP:/目标路径/
rsync -avzP hz_varlidar_image.tar.gz user@目标服务器IP:/目标路径/ # another way

## at the remote
gunzip hz_varlidar_image.tar.gz
docker load -i hz_varlidar_image.tar


### 容器内的数据会保留在镜像中，但主机挂载的目录（如 /data0/dataset）需在目标服务器上手动同步。
### 使用对应的目录进行启动
docker run -it --name hz_varlidar --shm-size 16g \
  -v /data0/dataset/:/data \
  -v /home/ljm/hz:/home/hz \
  --dns=8.8.8.8 \
  hz_varlidar_image:1.0 \
  /bin/bash
```
